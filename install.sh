...
DOWNLOAD_HOST="https://github.com/amzeros/aaaaaaa"

DOWNLOAD_STANDBY="https://github.com/amzeros/aaaaaaa"

PATH_KT="/root/FCMinerProxy"

PATH_EXEC="FCMinerProxy"

PATH_CACHE="/root/FCMinerProxy/.cache"

PATH_LICENSE="/root/FCMinerProxy/license"

PATH_CONFIG="/root/FCMinerProxy/.env"

PATH_NOHUP="/root/FCMinerProxy/nohup.out"
PATH_ERR="/root/FCMinerProxy/err.log"
...
turn_on() {
    ...
    if [ ! -f "$PATH_TURN_ON_SH" ];then
        ...
        echo '  echo "FCMinerProxy已启动"' >> $PATH_TURN_ON_SH
        echo 'else' >> $PATH_TURN_ON_SH
        echo '  if [ $COUNT -ne 0 ]; then' >> $PATH_TURN_ON_SH
        echo '      echo "FCMinerProxy已启动, 无需重复启动"' >> $PATH_TURN_ON_SH
        echo '  elif [ $(id -u) -ne 0 ]; then' >> $PATH_TURN_ON_SH
        echo '      echo "使用ROOT用户登录才能启动FCMinerProxy"' >> $PATH_TURN_ON_SH
...
installapp() {
    ...
    wget -P $PATH_KT "${DOWNLOAD_HOST}/minerproxy_vcu-fc-${VERSION}_linux" -O "${PATH_KT}/${PATH_EXEC}" 1>/dev/null

    filterResult $? "拉取程序 minerproxy_vcu-fc-${VERSION}_linux"
...
check_hub() {
    ...
    tail -f /root/FCMinerProxy/nohup.out
}

check_err() {
    colorEcho ${YELLOW} "按住CTRL+C后台运行"
    tail -f /root/FCMinerProxy/err.log
}
...
echo "-------------------------------------------------------"
colorEcho ${GREEN} "欢迎使用FCMinerProxy安装工具, 请输入操作号继续。"
